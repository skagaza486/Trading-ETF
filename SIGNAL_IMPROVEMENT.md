...
## 現有閾值（v3 — 2026-06-18 架構重設計，2026-06-19 Tier 系統 + LONG_VCP 修復）
...
LONG_VCP (entry trigger — volatility contraction):           ← 2026-06-19 修復（EXP-014）
  aboveEma200 != false AND EMA20 > EMA50 AND nearHigh52w != false  ← 新增結構過濾
  AND atrSlope50 < 0 AND rvolRecentAvg10 < 1.0           ← rvolRecentAvg10 0.8→1.0（放寬）
  AND breakout20d = true AND RVOL > 1.5 AND CLV > 0.6
  AND RSI > 50                                            ← 新增動量確認
  AND regime != short_friendly
  AND EMA50 > EMA150 (OR null)                           ← HYP-020: 多時間框架 MA 對齊
  [hysteresis (prior bar in long ladder) 已移除 — atrSlope50 < 0 已提供充足時間脈絡]

LONG_BOUNCE (entry trigger — pullback reversal):           ← 2026-06-19 Tier-aware（EXP-015）
  ...
  AND close > EMA20 AND CLV > 0.6
  T1（growth）: RSI 46–58, pullbackRvolAvg < 0.9 (OR null), relStrengthVsSpy > 0.02
  T2（defensive）: RSI 42–58, pullbackRvolAvg < 1.2 (OR null), relStrengthVsSpy > 0
...
── RS Line indicators（HYP-026~027，待實作）────────────────────────────────────
rsLine:               close / SPY_close — 個股相對大盤的比值時間序列（IBD RS Line）
rsLineEma50:          EMA(rsLine, 50) — RS Line 中期趨勢均線
rsLineAboveEma:       rsLine > rsLineEma50 — RS 趨勢健康旗標（用於 LONG_BOUNCE 排雷）
rsLineNewHigh120d:    rsLine >= max(rsLine, last 120 bars) — RS Line 創 120 日新高旗標（研究 tag）
```
...
- **2026-06-22 決定：條件凍結。** n=10 下任何新增 filter 都是在縮小無意義結論的 n。唯一正確行動是等待 signal 自然積累和擴大 universe。

#### LONG_BOUNCE

- **已落地（2026-06-18）**：`pullbackRvolAvg < 1.2`（HYP-018）
- **已落地（EXP-015，2026-06-19）**：Tier-aware 條件 — T1 RSI≥46 / rvolThresh<0.9 / RS>2%；T2 維持原條件
- 當前數據（EXP-015，signal-winrate 299 stocks）：T1 n=86 avg5D=+1.57%；T2 n=366 avg5D=+0.67%；整體 n=452 avg5D=+0.84%，WinRate5D=55%
- 關鍵學習：Growth stocks (T1) 的 EMA20 bounce 比 defensive (T2) 更容易 fakeout；需要更嚴格的質量過濾（RS>2%、低量回調、RSI 底部更高）
- 待觀察：MAE 在 299-stock universe 下的表現；Tier-aware 條件是否解決了 EXP-013 觀察到的 G6 問題

#### LONG_VCP

- **已落地（EXP-014，2026-06-19）**：移除 hysteresis；加入 `ema20>ema50` / `nearHigh52w` / `rsi14>50`；放寬 `rvolRecentAvg10 < 0.8` → `< 1.0`
- 當前 gate（EXP-014，signal-winrate 299 stocks）：n=50，avg5D=+0.54%，vs SPY=+0.34%，WinRate5D=52%
- 根因確認：hysteresis 與 atrSlope50<0 形成雙重過濾；移除後 n 從 6→50（+733%），方向穩定為正
- 與 LONG_BREAK 的差異：LONG_BREAK 用 RVOL/CLV/CMF 組合；LONG_VCP 用 atrSlope50/rvolRecentAvg10 描述量能收縮，加入 nearHigh52w 限制發生位置
- 待研究：n=50 仍未達 G1（100）；繼續擴大 watchlist；觀察 T1/T2 tier 對 LONG_VCP 的影響
...
## 研究誠信優先序（2026-06-22 重排）

> 框架：先修驗證，再修模型。以下是 skeptical reviewer 角度的分析結論。
> 核心原則：在 D1 資料完整性問題修好之前，任何「avg5D 變了 0.3%」的調 threshold 決定都是在調一個有偏差的數字。

### 現在應先做（Realism / Validation Work）

1. **HYP-013 P0 — D1 earnings archive rollout 驗證（核心修復已落地）**
   - **現況更正（2026-06-23）**：先前記錄的 `cronSnapshot.ts` 缺第 4 參數問題，現行代碼已修正；`runBackfillChunk()` 已會先讀取 D1 的 `historicalEarningsMap`，再傳入 `buildHistoricalSignals(..., historicalEarningsMap)`
   - `researchAgent.ts`、`scripts/build-snapshot.ts`、`worker.ts` ingest path、`src/worker/cronSnapshot.ts` backfill path 現已對齊同一條 earnings-aware research pipeline
   - `earnings_calendar` 表與 ingest/backfill 支援已落地；真正待確認的是 production D1 是否已完成歷史資料回填
   - 現在的重點不再是「補這行代碼」，而是驗證 historical `signals.earnings_in_window` 比例是否回到合理區間（預期約 ~11%），以及確認 Gate Summary / research output 是否已脫離該偏差

2. **HYP-012 doc fix — 文件落後實作（5 分鐘）**
   - 代碼在 `stockResearchEngine.ts:16-17` 和 `54-55` 已用 `nextBar?.open ?? close`
   - 文件寫「待實驗」是錯的，已自動落地

3. **LONG_BREAK 條件凍結（立即決定，2026-06-22）**
   - n=10 下任何 gate 結果都是統計噪音；任何新增 filter 只是把無意義的結論壓縮到更小的 n
   - 唯一有意義的行動：等待 signal 自然積累 + 繼續擴 universe

### 可以延後（Signal Tuning Work）

1. **HYP-014 — 真正的 walk-forward（6-12 個月後）**
   - LONG_BREAK n=10 無法做 train/test 分離；LONG_VCP n=50 做出來也無統計意義
   - 需要 LONG_BOUNCE n≥300 且跨越多個 regime（含非牛市）；現在做工程成本高但資訊增益低

2. **HYP-025/027 — RS 相關指標升級**
   - 方向合理，但在 HYP-013 修好之前，測試結果會混入 earnings 偏差

3. **HYP-015 — frozen universe snapshot**
   - ML 訓練（Track A 實際執行）前的前置條件；不需要現在開始

### 暫時不應投入

1. **任何新的 LONG_BREAK filter**（ADX、RVOL 調整、新 structural condition）— n=10 功率接近零
2. **LONG_BOUNCE MAE entry filter** — EXP-013 已確認是結構性特徵，靠 position sizing
3. **Short side 信號優化** — 需要 short_friendly regime 樣本（2022 熊市數據）
4. **ML 實際訓練** — HYP-013 earnings contamination + HYP-015 survivorship bias 未修復前，餵給更強模型只是複製偏差
...
**狀態：** ✅ 已落地（自動，2026-06-22 確認）

**確認路徑：** `stockResearchEngine.ts:16-17` — `const nextBar = history.bars[index + 1]; const entry = nextBar?.open ?? history.bars[index]?.close`；`stockResearchEngine.ts:54-55` — `const entryPrice = nextBar?.open ?? closeAtSignal`。兩條路徑都用 next-bar open，fallback 到 close 僅在最後一根 bar 無 nextBar 時觸發（合理邊界）。

**結果：** next-open entry 已全面落地，無需額外代碼改動。文件此前錯誤標記為「待實驗」。

---

#### HYP-013 — 歷史 replay 納入 earnings archive

**狀態：** ✅ **RESOLVED（2026-06-23）— SEC Edgar 替換 Finnhub，earnings_ratio_pct 升至 3.12%**

**修復摘要：**

- Finnhub `calendar/earnings` API 對 239/299 symbols 返回錯誤（原始 root cause，已確認）
- **修復方案：** 替換為 SEC Edgar 免費 API（`data.sec.gov/submissions`）
- **結果：** `earnings_ratio_pct` 從 0.02% 升至 **3.12%**；275/299 CIKs 已映射；`earnings_calendar` 覆蓋顯著改善
- D1 schema / ingest path / backfill logic 全部無需修改（只換了 fetch 端點）

**驗收數字（2026-06-23）：**

- `earnings_ratio_pct`: ~3.12%（目標 ~11%，但已大幅改善，剩餘差距是 SEC Edgar 只有近期財報，歷史覆蓋仍有限）
- `earnings_calendar` CIK coverage: 275/299

**注：** earnings_ratio_pct 仍低於理論 ~11%，因 SEC Edgar 歷史財報密度本身有限。此偏差已大幅降低，不再是 ML 訓練的主要污染源。若需進一步改善，可考慮 paid data source（Polygon.io / Refinitiv）。
...
**狀態：** 6-12 個月後目標（現在樣本量不足，LONG_BREAK n=10 無法分 train/test）

**假設：** 目前 Gate 是全樣本 pooled。改成多 rolling window 多數決，能排除只在少數子區間有效的脆弱規則。

**2026-06-22 分析：** 現有 `evaluateRollingWindowRobustness` 是 sliding-window in-sample evaluation，不是真正的 train/test 分離 walk-forward——每個 window 都在自己的數據上計算，沒有 held-out test period。用戶診斷正確。但實際制約是樣本量：LONG_BREAK n=10 根本無法分割；LONG_VCP n=50 分割後 test period 無統計意義；只有 LONG_BOUNCE（n≈452）才有條件做有意義的 walk-forward。前提條件：需要跨越多個 regime（含非牛市）的數據，目前 2 年牛市樣本不足。

---

#### HYP-015 — 建立 frozen universe snapshot

**狀態：** ML 訓練前 P0；核心基礎已落地，待歷史覆蓋 / consumer verification（2026-06-23 更新）

**現況更正：**
- 這不再是「尚未開始」的前置工程
- `watchlist_universe_snapshots` 表、寫入邏輯、批次 ingest endpoint、以及 point-in-time 查詢邏輯都已存在於現行代碼

**已落地基礎設施：**
- `schema/d1-migrate-r9-research-data.sql` 已建立 `watchlist_universe_snapshots`
- `worker.ts` 在 daily ingest / cron 路徑已寫入當期 universe snapshot
- `worker.ts` 已提供 `/api/admin/universe-snapshots` 批次 ingest endpoint
- `/api/d1/signals` 已支援 `point_in_time=1`，會依 `signalDate` 讀取對應月份的 universe membership
- `package.json` 已有 `research:backfill-universe` 補歷史月份資料的腳本路徑

**真正剩餘工作：**
- 確認 production D1 的歷史月份 universe snapshots 是否已補齊
- 確認需要 research integrity 的查詢 / consumer 確實有使用 point-in-time 模式，而不是默認沿用「當前 watchlist」
- `scripts/backfillUniverseSnapshotsFromGit.mjs` 現已支援 sparse manual snapshots carry-forward；後續應只重建「起始月份 + watchlist 變更月份」，避免逐月手填

**研究影響評估（原判斷仍成立）：**
- 主要偏差是 selection bias（加入 watchlist 帶有前視偏差），而非退市型 survivorship bias
- 對現有 signal gate 研究的影響屬二級，但對 ML / feature matrix 屬第一級問題
- **在 Track A / ML 實際執行前，必須確認歷史 snapshot 覆蓋與 point-in-time 消費都已完成**
...
#### HYP-025 — Universe Filter RS 定義升級（126d return diff）

**狀態：** 待實驗

**背景：** 現有 `relStrengthVsSpy` 是 20 天 rolling return diff。Minervini Trend Template 條件 8 的 RS > 70 指的是長期橫截面動量（過去 6-12 個月），而非 20 天差值。20 天 RS 容易被短期超跌反彈污染，讓「近期急反但長期弱勢」的股票誤入 LONG_BASE / WATCH 候選池。

**市場假設：** 將 RS 窗口從 20 天延長至 126 天（半年），更能篩出「持續跑贏大盤的真正領頭羊」，提升候選池質量，間接提高後續觸發 LONG_BREAK / LONG_BOUNCE 的信號品質。

**實作路徑（Path B — 無架構改動）：**

- 新增 indicator：`relStrengthVsSpy126d: number | null` — 126 日 rolling return diff（vs 20 日）
- 修改 `LONG_BASE` filter：`relStrengthVsSpy > 0` → `relStrengthVsSpy126d > 0`
- 修改 `WATCH` filter：`relStrengthVsSpy > -0.02` → `relStrengthVsSpy126d > -0.02`

**注意：** 完整的橫截面排名（IBD RS Rating 1-99）需要所有 universe 股票同日數據，要求雙 pass 架構改動，暫不採用。Path B 已捕捉長期超跑大盤的核心概念，工程成本接近零。

**接受標準：** LONG_BASE 和 WATCH 的候選池規模變化合理（可接受縮小 20-30%）；後續觸發 LONG_BREAK 的樣本，其 avg5D vs SPY 不低於當前水準。

**風險：** LONG_BREAK n=10 可能因候選池收窄進一步降低。需在實作後先觀察 n 變化再評估是否接受。

---

#### HYP-026 — RS Line 趨勢作為 LONG_BOUNCE 排雷機制

**狀態：** 已落地（EXP-012 驗證）— **MAE 控制無效，降級為 research tag**

**動機：** LONG_BOUNCE 當前 n=370、G6 FAIL（MAE=3.0%）。很多回踩 EMA20 的反彈最終失敗，是因為該股票相對於大盤正在走弱（RS Line 跌破其自身 EMA50）——這類「相對弱勢股的死貓跳」風險極高。IBD 強調，健康洗盤的 RS Line 應保持完好，向上的 RS Line 意味著機構仍在積累，向下則是出貨跡象。

**市場假設：** 在 `recentPullbackNearEma20` 觸發時，若 RS Line（= close / SPY_close）已跌破其 50 日 EMA，該回調反彈是機構出貨的「死貓跳」，而非健康洗盤。排除此類股票應使 MAE 改善。

**EXP-012 驗證結果（2026-06-18）：**

- n: 370 → 272（過濾 98 筆 RS 弱勢）
- vs SPY: +0.9% → +1.3%（方向改善）
- MAE: 3.0% → 3.1%（無效，微升）— G6 持續 FAIL
- 結論：rsLineAboveEma 過濾的並非 MAE 最差的那批 entry；MAE 問題根源在 entry timing，非 RS 方向

**接受標準（未達）：**

- G6 MAE < 2.5%（實測 3.1%）— FAIL
- vs SPY 維持 > +0.8% — PASS（+1.3%）
- n ≥ 200 — PASS（272）

**工程備注：** rsLine 序列可在 `buildIndicatorSnapshot` 內直接計算（`bars[i].close / spyBars[i].close`），SPY 已在 benchmarks 中可用，無架構改動。需確認 stock bars 與 SPY bars 的 date 對齊邏輯與現有 `relStrengthVsSpy` 一致。

---

#### HYP-027 — RS Line 創新高作為 Research Tag

**狀態：** 待實作（隨 HYP-026 一併完成，零額外工程成本）

**動機：** IBD 最強買點是「價格突破 + RS Line 創新高」。不應寫死為 rule（以免錯失非典型牛股），而應作為 feature 記錄，為未來 Meta-Labeling（Stage C）積累數據。

**改動（待實作）：**

- 新增 indicator：`rsLineNewHigh120d: boolean | null` — 今日 rsLine 是否創過去 120 bar 新高
- 不修改任何現有 trigger
- 研究方向：在 Gate Summary 的維度分析中，觀察帶有 `rsLineNewHigh120d == true` 的 LONG_BREAK / LONG_VCP 樣本，其 avg5D vs SPY 是否顯著高於 `false` 的樣本

**接受標準：** 純 Research Tag，不評估 gate。成功標準是數據收集完整、欄位在 snapshot 中可查。
...
### EXP-012 — HYP-026 RS Line 過濾效果驗證（LONG_BOUNCE MAE 控制）

- 日期：2026-06-18
- 對應假設：HYP-026（`rsLineAboveEma !== false` 作為 LONG_BOUNCE 排雷）
- 改動：`signalClassifier.ts` LONG_BOUNCE 加入 `indicators.rsLineAboveEma !== false`（null-safe）
- Baseline（EXP-011）：n=370，vs SPY +0.9%，MAE=3.0%，G6 FAIL
- 改動後 Gate Summary（2026-06-18 auto-run）：

| Label | n | Avg 5D | vs SPY | MAE 5D | G1 | G3 | G6 | Status |
| --- | ---: | ---: | ---: | ---: | --- | --- | --- | --- |
| LONG_BREAK | 10 | +2.5% | +2.3% | 2.3% | FAIL | PASS | PASS | INSUFFICIENT |
| LONG_VCP | 6 | +1.5% | +1.1% | 3.5% | FAIL | NA | NA | INSUFFICIENT |
| LONG_BOUNCE | 272 | +1.7% | +1.3% | 3.1% | PASS | PASS | FAIL | FAIL |

- 結論：FAIL — HYP-026 未達目標。n 從 370→272（過濾 98 筆 RS 弱勢），vs SPY 改善（+0.9%→+1.3%），但 MAE 反而從 3.0%→3.1%（微升）。rsLineAboveEma 過濾的是相對弱勢股，但並非 MAE 最差的那批，故 MAE 控制效果為零。
- 下一步：rsLineAboveEma 降級為 research tag（HYP-027 方向），不作硬性 gate；LONG_BOUNCE MAE 控制需從 entry timing 著手（e.g. CLV floor 提高、更嚴格的 recentPullbackNearEma20 定義）

---

### EXP-013 — LONG_BOUNCE MAE 控制系列驗證

- 日期：2026-06-18
- 對應假設：HYP-028 系列（entry 側過濾是否能降低 LONG_BOUNCE MAE）
- Baseline（EXP-011）：n=370，vs SPY +0.9%，MAE=3.0%，G6 FAIL

測試一：CLV floor 0.6→0.7（HYP-028a）

結果：n=202，MAE=3.1%。MAE 無改善，n 大幅下降。FAIL。

測試二：ema20Slope > 0（HYP-028b）

市場假設：短期趨勢向上時反彈才算健康，否則是接刀。結果：n=316，vs SPY +0.8%，MAE=3.1%。MAE 無改善，G3 降至邊界。FAIL。

Rolling Robustness（HYP-028b 狀態下）：

| Window | G2 Pass | G3 Pass | G6 Pass | Full PASS |
| --- | ---: | ---: | ---: | ---: |
| 6M | 6/6 | 6/6 | 3/6 | 2/6 |
| 12M | 6/6 | 6/6 | 3/6 | 2/6 |
| 18M | 6/6 | 6/6 | 3/6 | 2/6 |

- 結論：**EXP-013 關閉 — entry 側過濾無法解決 LONG_BOUNCE MAE 問題**。G6 在 3/6 rolling windows 通過，說明 MAE 是 regime 週期性問題（volatile 環境下 EMA20 反彈天然有更大逆向波動），而非個股選擇問題。任何 entry filter 只是在各 regime 中隨機切片，無法針對性移除高 MAE 的市況。G2/G3 全 6/6 rolling 說明方向和超額回報穩定可靠。
- 學到的原則：**LONG_BOUNCE 的 MAE 控制應靠 position sizing（小於 LONG_BREAK），而非更嚴格的 entry 條件**。G6 threshold（< 3%）對 mean-reversion trade 可能過嚴——breakout 型 trade 可從 entry 就獲利，而 bounce 型 trade 天然有「先跌再升」的特性。
- 下一步：保留 LONG_BOUNCE 現有定義（回退至 EXP-011 baseline，n=370），接受 G6 FAIL 為已知限制，在 UI 說明中標注「mean-reversion，建議小倉」；MAE 議題留待 B2 架構後以 Triple-Barrier Method 重新標籤再評估。
- 代碼狀態：所有 HYP-028 改動已 revert，signalClassifier 回到 EXP-011 基線

---

### EXP-014 — LONG_VCP 方向修正（移除 hysteresis，加入結構過濾）

- 日期：2026-06-19
- 對應假設：EXP-011 的遺留問題（LONG_VCP n=6，樣本太少，方向仍不穩定）
- 問題根因：VCP 定義缺乏結構過濾（無 ema20>ema50、無 nearHigh52w、無 rsi14>50），hysteresis（prior bar in long ladder）與 atrSlope50<0 形成雙重過濾，反而排除了有效 VCP 信號
- 改動：
  - 移除 hysteresis（`previousLabel` 不再為硬性條件）
  - 加入 `ema20 > ema50`（確認中期上升趨勢）
  - 加入 `nearHigh52w !== false`（VCP 應發生於 52 週高位附近）
  - 加入 `rsi14 > 50`（動量確認）
  - 放寬 `rvolRecentAvg10 < 0.8` → `< 1.0`（VCP 量能收縮不需要極端低量）
  - 保留 `EMA50 > EMA150`（null-safe）+ `atrSlope50 < 0` + `CLV > 0.6` + `RVOL > 1.5`
- 改動前（EXP-011 baseline）：n=6（實為 n=8 去除 hysteresis 後），avg5D=-2.45%，vs SPY 負向
- 改動後（signal-winrate.mjs，299 stocks，2yr）：n=50，avg5D=+0.54%，vs SPY=+0.34%，WinRate5D=52%
- 結論：**KEEP** — 方向完全修正（從負到正）。n 從 6→50（+733%）。根因確認：hysteresis + 缺乏結構過濾導致 VCP 捕捉的是「無上升趨勢背景的噪音突破」
- 下一步：n=50 仍未達 G1（100）；watchlist 持續擴展；LONG_VCP 與 LONG_BREAK 重疊度觀察

---

### EXP-015 — LONG_BOUNCE Tier-aware 條件（T1 growth 收緊）

- 日期：2026-06-19
- 對應假設：EXP-013 的後繼——entry 側過濾無效，但 universe 分層可能有效
- 背景：Watchlist 擴展至 299 stocks 後，signal-winrate 顯示 LONG_BOUNCE 在 T1 growth stocks 表現遠差於 T2 defensive stocks。假設：high-volatility growth stocks 的 EMA20 回踩更容易 fakeout，需要更嚴格的過濾
- Tier 定義：T1=123 growth/momentum stocks（NVDA, PLTR, META, TSLA 等），T2=176 defensive/value stocks（Utilities, Staples, REIT, Materials, Energy, 成熟 Tech 等）
- 改動：
  - T1 新增：RSI ≥ 46（vs T2 42），pullbackRvolAvg < 0.9（vs T2 1.2），relStrengthVsSpy > 0.02（vs T2 > 0）
  - T2 條件：維持原 EXP-011 baseline（RSI 42–58，pullbackRvolAvg < 1.2，RS > 0）
- 改動前（signal-winrate 基線，299 stocks）：LONG_BOUNCE T1=n=86 avg5D=-0.11%；T2=n=366 avg5D=+0.67%；整體 n=452 avg5D=+0.36%
- 改動後（tier-aware 條件）：

| 組別 | n | Avg5D | vs SPY | WinRate5D |
| --- | ---: | ---: | ---: | ---: |
| LONG_BOUNCE T1 | 86 | **+1.57%** | +1.09% | **64%** |
| LONG_BOUNCE T2 | 366 | +0.67% | +0.48% | 53% |
| LONG_BOUNCE 整體 | 452 | +0.84% | +0.60% | 55% |

- 結論：**KEEP** — T1 方向完全修正（-0.11% → +1.57%），T2 保持穩定。假設成立：growth stocks 的 EMA20 bounce 需要更嚴格的質量過濾（RS>2%、低量回調、RSI 底部更高）
- 學到的原則：**不同 volatility profile 的股票應有不同的 bounce 閾值**；tier 分層是解決此問題最低工程成本的路徑
- 代碼實作：`signalClassifier.ts` `resolveStockLabel(tier: 1|2=1)` → tier-aware `rsiLow` / `rvolThresh` / `rsThresh`；`stockScreenerEngine.ts` / `cronSnapshot.ts` / `App.tsx` 全鏈路傳遞 tier

---

## 版本快照

### v5 EXP-015 — 2026-06-19（Watchlist 299 stocks，Tier-aware LONG_BOUNCE，LONG_VCP 修復）

| Label | n | Avg5D | vs SPY | WinRate5D | G1 | G2 | G3 | 狀態 |
| --- | ---: | ---: | ---: | ---: | --- | --- | --- | --- |
| LONG_BOUNCE T1 | 86 | +1.57% | +1.09% | 64% | PASS | PASS | PASS | 待 MAE 驗證 |
| LONG_BOUNCE T2 | 366 | +0.67% | +0.48% | 53% | PASS | PASS | PASS | 待 MAE 驗證 |
| LONG_BOUNCE 整體 | 452 | +0.84% | +0.60% | 55% | PASS | PASS | PASS | 待 MAE 驗證 |
| LONG_VCP | 50 | +0.54% | +0.34% | 52% | FAIL（n=50） | PASS | PASS | INSUFFICIENT |
| LONG_BASE | 2886 | +0.42% | +0.15% | 53% | — | — | — | universe filter |

（數據來源：signal-winrate.mjs，299 stocks，2yr，batched fetch concurrency=6）

關鍵變化：LONG_VCP 方向由負轉正（EXP-014）；LONG_BOUNCE T1 從 -0.11% 升至 +1.57%（EXP-015 tier-aware）；整體 LONG_BOUNCE 從 +0.36% 升至 +0.84%。Watchlist universe 從 130 → 299 stocks。

---

### v4 EXP-013 — 2026-06-18（LONG_BOUNCE MAE 系列研究後，回退至 EXP-011 基線）
...
關鍵發現：MAE 3.0% 是 LONG_BOUNCE 的結構性特徵（mean-reversion trade），非可過濾的個股問題。G6 3/6 rolling windows pass 確認。接受此限制，MAE 控制應靠 position sizing。

### v3 EXP-012 — 2026-06-18（HYP-026 RS Line 過濾後）

| Label | n | vs SPY | MAE | G1 | G2 | G3 | G6 | 狀態 |
| --- | ---: | ---: | ---: | --- | --- | --- | --- | --- |
| LONG_BREAK | 10 | +2.3% | 2.3% | FAIL | PASS | PASS | PASS | INSUFFICIENT |
| LONG_VCP | 6 | +1.1% | 3.5% | FAIL | NA | NA | NA | INSUFFICIENT |
| LONG_BOUNCE | 272 | +1.3% | 3.1% | PASS | PASS | PASS | FAIL | FAIL |

關鍵觀察：HYP-026 過濾了 98 筆 RS 弱勢 BOUNCE，vs SPY 從 +0.9% 升至 +1.3%，但 MAE 3.1% 未改善（G6 持續 FAIL）。LONG_BREAK n=10 維持。MAE 控制需換方向。
