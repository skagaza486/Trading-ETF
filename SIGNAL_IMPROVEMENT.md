...
1. **HYP-013 P0 — D1 backfill earnings 缺口（已確認 bug，非假設）**
   - `cronSnapshot.ts:523` 呼叫 `buildHistoricalSignals(allHistories, tickers, 250)` — 缺第 4 參數
   - `researchAgent.ts:478-482` 正確傳入 `historicalEarnings` — 兩條路徑不一致
   - 結果：D1 所有 historical signal 的 `earnings_in_window` 永遠是 `false`，~11% earnings-adjacent 訊號未被標記，Gate Summary 有系統性樂觀偏差
   - 修復需要資料管線改動（不只代碼）：需把歷史 earnings calendar 納入 GitHub Actions build 或 backfill endpoint
...
**狀態：** P0 — 已確認代碼分歧（2026-06-22，非假設）

**已確認的 bug：**

- `cronSnapshot.ts:523`：`buildHistoricalSignals(allHistories, tickers, 250)` — **缺第 4 參數**（historicalEarningsMap）
- `researchAgent.ts:478-482`：`buildHistoricalSignals(histories, tickers, RESEARCH_SIGNAL_BARS, historicalEarnings)` — 正確傳入

**結果：** D1 backfill 所有 historical signal 的 `earnings_in_window` 永遠是 `false`。`buildHistoricalSignals` 內部 `historicalEarningsMap?.get(ticker) ?? []` 回傳空陣列 → `nextEarnings = null`，信號不被標記為 earnings-adjacent。

**影響評估：** 每檔股票每 90 天有一次財報，5 日窗口 ≈ 11% 交易日。約 11% 的 D1 signal 應被標為 `earnings_in_window = true` 但沒有被標記。財報窗口 forward return 分布天然不同（波動更大、更隨機），混入主樣本導致 Gate Summary avg5D 和 MAE 有系統性樂觀偏差。

**修復複雜度：** 中。純代碼改動不夠——backfill 在 Worker 環境，需要歷史 earnings calendar 資料來源。建議路徑：GitHub Actions `build-snapshot.ts` 為每個 ticker fetch 過去 2 年 earnings dates，存入 D1 `earnings_calendar` 表；backfill endpoint 讀取此表傳入 `buildHistoricalSignals`。

**在此修好之前：** Gate Summary 的每個數字都有已知偏差，不應根據 avg5D 細微變化做條件調整決定。
...
**狀態：** ML 訓練前 P0；現在是二級問題（現有 signal 定義工作不受影響）

**假設：** 研究 replay 目前只對現在的 watchlist 回放，存在 survivorship bias。先記錄每月 universe snapshot，再按 signalDate 對應正確的 universe。

**2026-06-22 評估：** 主要偏差是 selection bias（加入 watchlist 帶有前視偏差），而非退市型 survivorship bias（我們的 US large/mid-cap 成份退市率極低）。對現有 signal gate 研究的影響是二級的——但對 ML 訓練（Track A）是第一級問題：把有 selection bias 的 feature matrix 餵給 LightGBM，只是用更強的模型過擬合一個有偏差的資料集。**必須在 Track A 實際執行前完成**，現在不需要開始。
