...
- 新增 indicators：`rsLine`、`rsLineEma50`、`rsLineAboveEma`（詳見 stockScreenerEngine 區塊）
- LONG_BOUNCE trigger 加入：`AND rsLineAboveEma != false`（null-safe：歷史不足時不排除）

**接受標準：**

- G6 MAE < 2.5%（從 3.0% 改善）
- vs SPY 維持 > +0.8%（允許輕微下滑）
- n 允許降至 250（-32%），不得低於 200
