...
| Portfolio v2 refactor (Zone D) | **Claude** | DeepSeek | ✅ Done 2026-06-24 (deployed `trading-etf` d0b354b8) — **tool/plan separation**: new `portfolioConfig.ts`, risk limits as `% of capitalBase`, personal 3-phase plan now DATA (`BUILTIN_PRESETS`), store→`portfolio_v3` (built-in presets auto-resolve from code on load). **All-USD** (presets HKD→USD @~7.8). **Sub-tabs** `組合 / ETF 配置 / 計劃參考`. **ETF Allocation Model** (`ETF_REFERENCE` sleeve/role/corr) — transparent, not a signal. Added §2 single-sector 25% alert. Known-deferred bugs noted in CLAUDE.md + HANDOFF_PORTFOLIO_UI. `tsc` clean. ⚠️ Zone D handoff to DeepSeek is paused while this lands — re-declare before next `src/web/` edit. |
